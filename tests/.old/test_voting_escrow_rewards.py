...
def test_receive_rewards(web3, chain, accounts, token, voting_escrow):
    ...
    assert voting_escrow.epoch() == 0
    assert voting_escrow.window_token_rewards(tx.timestamp // EPOCH_SECONDS * EPOCH_SECONDS, token) == reward_amount
    assert voting_escrow.window_token_rewards(tx.timestamp // EPOCH_SECONDS * EPOCH_SECONDS + EPOCH_SECONDS, token) == 0

    token.approve(voting_escrow, reward_amount)
    voting_escrow.receiveReward(token, reward_amount, {"from": payer})

    assert voting_escrow.epoch() == 0
    assert voting_escrow.window_token_rewards(tx.timestamp // EPOCH_SECONDS * EPOCH_SECONDS, token) == reward_amount * 2
    assert voting_escrow.window_token_rewards(tx.timestamp // EPOCH_SECONDS * EPOCH_SECONDS + EPOCH_SECONDS, token) == 0
    ...
    assert voting_escrow.window_token_rewards(tx.timestamp // EPOCH_SECONDS * EPOCH_SECONDS, token) == reward_amount * 2
    assert voting_escrow.window_token_rewards(tx.timestamp // EPOCH_SECONDS * EPOCH_SECONDS + EPOCH_SECONDS, token) == reward_amount
    ...
    assert voting_escrow.window_token_rewards(voting_escrow.currentWindow(), token) == reward_amount
...
def test_share_rewards_2users_same_time_deposit_gas_consumption(web3, chain, accounts, token, voting_escrow):
    EPOCH_SECONDS = voting_escrow.EPOCH_SECONDS()
    sleep = EPOCH_SECONDS
    MAXTIME = voting_escrow.MAXTIME()
    payer = accounts[0]
    ...
    assert claim_tx1.events['UserRewardsClaimed']['amount'] // 1000 == int(reward_amount * user1_share) // 1000
    assert voting_escrow.user_token_claimed_window(user1, token) == voting_escrow.currentWindow() - EPOCH_SECONDS
    assert claim_tx1.gas_used < 200_000
    ...
    assert claim_tx2.events['UserRewardsClaimed']['amount'] // 1000 == reward_amount * user2_share // 1000
    assert voting_escrow.user_token_claimed_window(user2, token) == voting_escrow.currentWindow() - EPOCH_SECONDS
    assert claim_tx2.gas_used < 200_000


def test_share_rewards_2users_same_time_deposit_gas_consumption_after_1_month_and_after_6_months(web3, chain, accounts, token, voting_escrow):
    EPOCH_SECONDS = voting_escrow.EPOCH_SECONDS()
    sleep = EPOCH_SECONDS
    MAXTIME = voting_escrow.MAXTIME()
    payer = accounts[0]
    ...
    user2_deposit_amount = deposit * 2
    user2_deposit_till = chain.time() + 365 * 24 * 3600
    token.transfer(user2, user2_deposit_amount)
    token.approve(voting_escrow, user2_deposit_amount, {"from": user2})
    tx_lock_user2 = voting_escrow.create_lock(user2_deposit_amount, user2_deposit_till, {"from": user2})
    assert voting_escrow.epoch() == 2
    ...
    print(f'start claim user1')
    claim_user1_tx1 = voting_escrow.claim_rewards(token, {"from": user1})
    claim_user1_tx1_window_start = claim_user1_tx1.events['UserClaimWindowStart']['value']
    claim_user1_tx1_window_end = claim_user1_tx1.events['UserClaimWindowEnd']['value']

    chain.sleep(5 * 30 * 24 * 3600)

    voting_escrow.claim_rewards(token, {"from": user1})
    ...
    print(f'start claim user1')
    claim_user1_tx2 = voting_escrow.claim_rewards(token, {"from": user1})
    claim_user1_tx2_window_start = claim_user1_tx2.events['UserClaimWindowStart']['value']
    claim_user1_tx2_window_end = claim_user1_tx2.events['UserClaimWindowEnd']['value']

    assert claim_user1_tx1.gas_used == claim_user1_tx2.gas_used


# def test_share_rewards_2users_same_time_deposit_claim_after_100_windows_gas_consumption(web3, chain, accounts, token, voting_escrow):
#     EPOCH_SECONDS = voting_escrow.EPOCH_SECONDS()
#     sleep = EPOCH_SECONDS
#     MAXTIME = voting_escrow.MAXTIME()
#     payer = accounts[0]
...
#     print(f"{claim_tx1.events=}")
#     assert claimable == claim_tx1.events['UserRewardsClaimed']['amount']
#     assert claim_tx1.gas_used < 100 * 20_000
#
#
# def test_share_rewards_2users_same_time_deposit_checkpoint_after_100_windows_gas_consumption(web3, chain, accounts, token, voting_escrow):
#     EPOCH_SECONDS = voting_escrow.EPOCH_SECONDS()
#     sleep = EPOCH_SECONDS
#     MAXTIME = voting_escrow.MAXTIME()
#     payer = accounts[0]
...
#     checkpoint_tx = voting_escrow.checkpoint({"from": user1})
#     assert checkpoint_tx.gas_used < 200_000


def test_share_rewards_2users_same_time_deposit__increase_amount(web3, chain, accounts, token, voting_escrow):
    EPOCH_SECONDS = voting_escrow.EPOCH_SECONDS()
    sleep = EPOCH_SECONDS
    MAXTIME = voting_escrow.MAXTIME()
    payer = accounts[0]
    ...
    token.transfer(user1, user1_deposit_amount)
    token.approve(voting_escrow, user1_deposit_amount, {"from": user1})
    tx_lock_user1 = voting_escrow.increase_amount(user1_deposit_amount, {"from": user1})
    user1_deposit_at = tx_lock_user1.timestamp
    epoch_after_user1_lock = voting_escrow.epoch()
    ...
    period_user1 = user1_deposit_till // EPOCH_SECONDS * EPOCH_SECONDS - tx_lock_user1.timestamp
    assert voting_escrow.user_point_history(user1, 0) == (0, 0, 0, 0)
    assert voting_escrow.user_point_history(user1, 2)[0] == (user1_deposit_amount // MAXTIME) * period_user1  # bias
    assert voting_escrow.user_point_history(user1, 2)[1] == (user1_deposit_amount // MAXTIME)  # slope
    assert voting_escrow.user_point_history(user1, 2)[2] == tx_lock_user1.timestamp  # ts
    assert voting_escrow.user_point_history(user1, 2)[3] == tx_lock_user1.block_number  # blk
    assert voting_escrow.user_point_history(user1, 3) == (0, 0, 0, 0)
    assert voting_escrow.point_history(epoch_after_user1_lock)[0] == (user1_deposit_amount // MAXTIME) * period_user1
    assert voting_escrow.point_history(epoch_after_user1_lock)[1] == (user1_deposit_amount // MAXTIME)
    assert voting_escrow.point_history(epoch_after_user1_lock)[2] == tx_lock_user1.timestamp
    assert voting_escrow.point_history(epoch_after_user1_lock)[3] == tx_lock_user1.block_number
...
def test_share_rewards_1user_MAX_TIME(web3, chain, accounts, token, voting_escrow):
    MAXTIME = voting_escrow.MAXTIME()
    deposit = 10**18
    user1 = accounts[1]
    ...
    token.transfer(user1, user1_deposit_amount)
    token.approve(voting_escrow, user1_deposit_amount, {"from": user1})
    voting_escrow.create_lock(user1_deposit_amount, user1_deposit_till, {"from": user1})


def test_share_rewards_2users_same_time_deposit__increase_unlock_time(web3, chain, accounts, token, voting_escrow):
    EPOCH_SECONDS = voting_escrow.EPOCH_SECONDS()
    sleep = EPOCH_SECONDS
    MAXTIME = voting_escrow.MAXTIME()
    payer = accounts[0]
    ...
    user1_deposit_amount = deposit
    user1_deposit_till = chain.time() + EPOCH_SECONDS*10*2
    token.transfer(user1, user1_deposit_amount)
    token.approve(voting_escrow, user1_deposit_amount, {"from": user1})
    tx_lock_user1 = voting_escrow.increase_unlock_time(user1_deposit_till, {"from": user1})
    user1_deposit_at = tx_lock_user1.timestamp
    epoch_after_user1_lock = voting_escrow.epoch()
...
#
#     # print(f"{tx.events['_averageUserBalanceOverEpochDebug']=}\n")
#     # print(f"{tx.events['_averageUserBalanceOverEpochDebugS']=}\n")
